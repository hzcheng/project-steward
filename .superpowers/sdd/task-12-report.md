...
- Controlled providers append JSONL records containing unique invocation ID, PID, exact cwd, and payload. Cleanup collects and deduplicates every PID in the ledger, with pid-file compatibility fallback, writes every controlled stop file, and performs only bounded `process.kill(pid, 0)` observation until all tracked processes exit. It never sends SIGTERM/SIGKILL, so stale or reused PIDs cannot cause destructive action. Provider and tmux fixture roots are removed independently only after their corresponding providers-stopped/server-stopped proof. The real smoke asserts the concurrent locators are identical, invocation count is one, project topology is exactly one managed session/two windows, and session topology is exactly two managed sessions/two rows.
...
- Provider cleanup no longer sends signals to ledger or pid-file PIDs. Stop files are the only control mechanism. Every tracked PID is polled with signal `0` until `ESRCH` within a fixed deadline; a live, stale/reused, or unverifiable PID produces `CleanupAggregateError` and retains the provider fixture root.
