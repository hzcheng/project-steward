...
export class OpenProjectStore {
    ...
    public constructor(rootDirectory: string, private readonly ownInstanceId: string) {
        ...
        this.instancesDirectory = path.join(rootDirectory, 'open-projects', 'v1', 'instances');
    }

    public async write(registration: OpenProjectRegistration): Promise<void> {
        const validated = validateOpenProjectRegistration(registration);
    ...
    public async scan(nowMs: number): Promise<OpenProjectStoreScan> {
        const counters: OpenProjectStoreScan['counters'] = {
            active: 0,
            parseErrors: 0,
            oversizedFiles: 0,
            symlinkFiles: 0,
        ...
        for (const entry of entries) {
            ...
            try {
                const registration = validateOpenProjectRegistration(
