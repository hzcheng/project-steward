...
| PASS (automated) | Cleanup failure boundaries | Pure injected checks distinguish explicit no-server results from ordinary numeric failures, retry an ordinary `kill-server` failure, and still run stopped verification plus later stages after two failures. Provider cleanup writes every controlled stop file and uses bounded `process.kill(pid, 0)` probes for every ledger/fallback PID; it never sends a destructive signal. Socket and fixture roots are removed only with independent server-stopped and providers-stopped proof. Any unverifiable root is retained for diagnosis before a local aggregate error compatible with older Node runtimes is returned. |
