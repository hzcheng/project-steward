...
interface AiSessionLifecycleSignal {
    ...
    attentionReason?: 'completed' | 'input-required' | 'failed' | 'aborted';
    occurredAtMs: number;
    token: string;
}
```
...
- Attention Monitor 只读取 `attentionReason` 及自身的确认状态。
- 一个完成事件可以同时输出 `stopped` 和 `completed`，但确认 `completed` 不会反向修改 execution state。

如果兼容迁移需要暂时保留现有 `phase` 字段，适配应局限在 lifecycle 边界，ACTIVE projection 不得继续依赖 attention phase 推导执行状态。
...
Normalized lifecycle signal
        ├───────────────┐
        ▼               ▼
Execution projector   Attention monitor
        │               │
        ▼               ▼
